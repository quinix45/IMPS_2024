...
Most asymmetric models include asymmetry parameters that are hard to estimate in small sample sizes [e.g., @Goncalves_etal_2023; @Lee_Bolt_2018a] Two recently proposed asymmetric IRT models [@Shim_etal_2023a; @Shim_etal_2023] may help address this issue:
...
::: {.fragment .fade-in fragment-index="3"}
The core of the **BMA** approach we implement is an approximation of leave-one-out cross validation [$LOO_{CV}$\; @Vehtari_etal_2017].
:::
</br>
...
<center> **Ideal Scenario: theoretical quantiles** </center>
...
<center> **Reality: empirical quantiles** </center>
...
The Bond's Logical Operations Test [BLOT\; @Bond_Fox_2007] dataset from the PsychTools package [@Revelle_2024], which includes **150 participants** and **35 items**. We fit the 1PL, 2PL, 1CLL, 2CLL, 1NLL, 2NLL. 
...
$$RMSE_{q} = \sqrt{\frac{1}{100}\sum_{r=1}^{100}\frac{1}{I}\sum_{i = 1}^{I}[P_r(y_{in} = 1|\color{red}{\theta_{q}}) - \hat{P}_r(y_{in} = 1|\tilde{\theta}_{q})]^{2}}$$
:::
...
$\sigma_{log(\bar{a})} = Exponential(3)$

$\bar{b} = N(0, 1)$

$\sigma_{\bar{b}} = Lognormal(.25, .5)$
...
![](images/Tab_datagen.png){fig-align="center"}
